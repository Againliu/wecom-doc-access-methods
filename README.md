...
| Version | Key Changes |
|---------|-------------|
| **v5.10.0** | **扫码授权体验固化**（来自 8-27 杨文丽案 8 轮返工复盘）：① 新增 `scripts/qr_to_wecom.py`——把 `wecom_auth_flow.py` 拦截的 1-bit PNG 二维码转成 1160×1160 白边 RGB JPEG（企微长按识别一次成功），实测通过。② SKILL.md 精简卡新增「扫码授权铁律」四条（--wait-done 入口 / 发图不发 URL / 等待期后台轮询不空转 / 同一 transaction_id 不重发），修复 8-27 分层改造时误删授权流程导致教训不可见。③ pitfalls.md 新增完整链路复盘条目（三层根因：链接不可用 + 1-bit 裂图 + 教训写错 skill）。**核心教训：教训写错 skill 等于没写——反思类准则要落到实际任务加载的那张卡上。** |
