...
class WeComDocReader:
    ...
    async def check(self, user_id: str) -> dict:
        ...
        v4.5.2: 用 HTTP 请求替代 Playwright 启动 — <100ms vs 3-5s，
        不依赖 Chromium 二进制，cron sanitized 环境兼容。
        企微文档是 SPA，HTTP 拿到的是 HTML 框架，但足够判断是否跳转 login 页。
        """
        state_file = str(self._state_file(user_id))
        ...
        # 动态检查：HTTP 请求验证（v4.5.2 — 替代 Playwright）
        # cookies 有效：status 200 + 未跳转 login
        # cookies 过期：302 跳转 login 或 body 含 login 标记
        valid = False
        detail = ""
        try:
            import httpx
            ...
            is_login_body = (
                "login" in resp.text[:3000].lower()
                and "scenario" in resp.text[:3000].lower()
            )

            valid = resp.status_code == 200 and not is_redirect_login and not is_login_body
            if not valid:
                if is_redirect_login:
                    detail = f"跳转到登录页 (302 → {resp.headers.get('location', '')[:80]})"
                elif resp.status_code != 200:
                    detail = f"HTTP {resp.status_code}"
                else:
                    detail = "页面内容含 login 标记"
        except Exception as e:
            valid = False
            detail = f"HTTP 请求失败: {type(e).__name__}: {e}"[:200]

        result = {"valid": valid, "user_id": user_id}
        if remaining_days is not None:
            result["remaining_days"] = round(remaining_days, 1)
        result["reason"] = "OK" if valid else detail
        return result
