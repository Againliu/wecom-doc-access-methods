...
def main():
    ...
    if args.cmd == "login":
        try:
            ...
            r = {"success": False, "error": f"{type(e).__name__}: {e}", "user_id": args.user_id}
    elif args.cmd == "check":
        try:
            r = asyncio.run(reader.check(args.user_id))
        except Exception as e:
            r = {"valid": False, "user_id": args.user_id, "reason": f"{type(e).__name__}: {e}"}
    elif args.cmd == "read":
        try:
            r = asyncio.run(reader.read(args.user_id, args.url, args.sheet))
        except Exception as e:
            r = {"success": False, "error": f"{type(e).__name__}: {e}", "url": args.url}
